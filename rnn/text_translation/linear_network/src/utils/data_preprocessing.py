...
def build_vocab(filepath, tokenizer):
    ...
    ) as f:  # need encoding to read in the filepath name
        for string_ in f:  # Read in each line in the file
            # print(f"TESTING + {string_}")
            counter.update(
                tokenizer(string_)
            )  # for every line read in file, update counter
    return Vocab(
        counter, specials=["<unk>", "<pad>", "<bos>", "<eos>"]
    )  # list of special paddings before the words
...
def data_process(filepaths):
    ...
    for (raw_de, raw_en) in zip(raw_de_iter, raw_en_iter):
        ...
        )  # torch.long = 64-bit signed interger
