...
class Encoder(nn.Module):
    ...
    def forward(self, src):
        ...
        embedded = self.dropout(self.embedding(src)) # embbedded vectorize the input sentence, embdded = [src len, batch size, emb dim]
        outputs, (hidden,cell) = self.rnn(embedded) # input embedded and output 3 things
...
class Decoder(nn.Module):
    def __init__(self, output_dim, emb_dim, hid_dim, n_layers, dropout): # used for define layer
        super().__init__()
    ...
    def forward(self, input, hidden, cell): # used for calculate forward pass
        ...
        embedded = self.drop_out(self.embedding(input))         #embedded = [1, batch size, emb dim]
        ...
        prediction = self.fc_out(output.squeeze(0)) #  get rid of the sentence length dim
...
class Seq2Seq(nn.Module):
    def __init__(self, encoder, decoder, device):
        ...
        assert encoder.hid_dim == decoder.hid_dim, "Hidden dim of encoder and decoder must be equal"
        assert encoder.n_layers == decoder.n_layers, "Encoder and decoder must have the same number of layer"
