...
class Decoder(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                DecoderLayer(hid_dim, pf_dim, dropout, device, n_heads)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, trg, enc_src, trg_mask, src_mask):
        ...
        masked_attention, _ = self.auto_attn(
            first_norm_out, first_norm_out, first_norm_out, trg_mask
        )
        first_gate_out = self.auto_gate(trg, self.dropout(masked_attention))

        second_norm_out = self.second_norm(first_gate_out)
        self_attention, attention = self.self_attention(
            second_norm_out, enc_src, enc_src, src_mask
        )
