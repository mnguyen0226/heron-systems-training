...
class Encoder(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                EncoderLayer(hid_dim, pf_dim, dropout, device, n_heads)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask):
        ...
        attn_out, _ = self.attn_layer(
            first_norm_out, first_norm_out, first_norm_out, src_mask
        )
...
class Attn(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        x = x.permute(
            0, 2, 1, 3
        ).contiguous()  # Change the order in the shape, make sure that the rearrange array still contiguous
...
class Gate(
    nn.Module
):  # make sure this has the input dim and the self attention dim to be the same size
    def __init__(self, input_dim):
        ...
        super().__init__()
        self.gru = nn.GRU(
            input_size=input_dim, hidden_size=input_dim
        )  # output the hidden state & gate output
