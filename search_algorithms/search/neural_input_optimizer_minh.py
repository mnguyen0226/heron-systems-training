...
# Script provides the code for random and greedy search algorithms
import copy
from abc import ABC
...
####################################################################################################
class _NeuralFloatInputOptimizer(object):  # Q: What is the objective of this class
    def __init__(self, net):
        self.net = net

    def _initialize_floats(self, population, n_self_units, safe_spawn):
        """Initialize starting location for all "self" units
        """
    ...
    def _gradient_mask(self, x, n_self_units):
        """Gradient Mask - Q: What does this do?
        """
    ...
    def batch_evaluate(
        ...
        """Batch Evaluation - Evaluate if the batch does not create too much army and still have slot for search
        """
        ...
        # TODO: this is hacky - this is not needed for the dashboard
...
####################################################################################################
class ArmySynthesizer(ABC): # - abstract base class
    OPTIMIZE_FLOAT_IXS = [ObsIdx.x, ObsIdx.y]  # Q: what is this? observation index
    ...
    def __init__(
        ...
        """Army Synthesizer Class contains the implementation of the RandomSearchSynthesizer (what we actually want)

        Parameters Q: Why are the gas, unit,... infinite
        ----------
        net:
            win_classifier_neural_network
        device:
            cpu or gpu
        available_units: List
            list of available unit. Q: Is this a given army initially?
        x_area: Tuple[int, int]
            length of the area
        y_area: Tuple[int, int]
            width of the ara
        minerals: int = np.Infinity
            number of available minerals
        gas: int = np.Infinity
            number of available gas
        units: int = np.Infinity
            number of available units
        map_size: tuple
            size of the game map
        seed: int
            seed for constant random generator.
        """
        ...
        # Army evaluator Q: what does this mean?
        self._evaluator = _NeuralFloatInputOptimizer(
            self.net,
        )

        # We don't care about x_max, y_max here Q: what does this mean?
        self._index_names = None

    def _create_scenario(self, raw_self_units, raw_non_self_units):
        """Create scenarios ? Q: What does this code do?

        Parameters Q: What does the parameters means
        ----------
        raw_self_units:
            ?
        raw_non_self_units:
    ...
    def evaluate_armies(self, samples, raw_non_self_units, safe_spawn, upgrades):
        """Evaluation Armies Q: What odes this code do?
    ...
    def synthesize(self, initial_state, upgrades):
        """Synthesize: Generate GUS army

        Parameters
        ----------
        initial_state:
            ? initial state of army?
        upgrades:
            ? upgrades army
        """
    ...
    @abstractmethod  # Abstract to allow writing the same-name function for both class
...
class RandomSearchSynthesizer(ArmySynthesizer):
    def __init__(
        ...
        self.max_evals = max_evals  # Q: What is this?

    def optimize(self, raw_non_self_units, safe_spawn, upgrades):
        """Just run the code to find

        Parameters Q: What does this code do and the parameter
        ----------
        raw_non_self_units:
            ? enermy
        safe_spawn:
        ...
        """
        # Generate the desired number of random samples from GUS
        samples = [self._gus.select() for _ in range(self.max_evals)]

        # Evaluate the samples from GUS
        evals = self.evaluate_armies(samples, raw_non_self_units, safe_spawn, upgrades)

        # Sort the samples by performance - for uniform it
...
class GreedySearchSynthesizer(ArmySynthesizer):
    def __init__(
        ...
        self._A = np.array([x for x, y in constraints], dtype=np.float32) # army
        self._b = np.array([y for x, y in constraints], dtype=np.float32) # budget

        self.partial_army = partial_army
        self.beam_size = beam_size # What is this?
    ...
    def optimize(self, raw_non_self_units, safe_spawn, upgrades):
        ...
        while len(fringe) > 0: # if able to add
        ...
        return sorted(solutions, key=lambda x: -x[1]) # Get the best army

####################################################################################################
class RandomGreedySearchSynthesizer(ArmySynthesizer):
    def __init__(
        self,
        net,
        device,
        ...
        seed: int = None,
        beam_size: int = 1,
        gus_percentage: float = 0.7, 
        top_armies_num: int = 25,
        greedy_levels: list = [1,2,3]
    ):
        """Random Greedy Search Synthesizer Algorithms that first randomly generate the army with GUS for 70% 
        of the army and then use Greedy Algorithms (evaluated by the win classifier) to provide the list of full
        army that give the highest percentage of winning

        Parameters
        ----------
        net:
            the win-classifier neural network
        device: String
            cpu/gpu
        available_unit: List[Union[unit_data.units.Terran, unit_data.units.Protoss, unit_data.units.Zerg]]
            list of available Terran, Protoss, or Zerg unit
        x_area: Tuple[int, int]
            coordinate x where to put the army
        y_area: Tuple[int, int]
            coordinate y where to pu the army
        minerals: int = np.Infinity
            the amount of minerals set as infinite
        gas: int = np.Infinity
            the amount of gas set as infinite
        supply: int = np.Infinity
            the amount of supply set as infinite
        units: int = np. Infinity
            the amount of unit set as infinite
        map_size: tuple = (64, 64)
            64x64 map size of the environment
        max_evals: int = 100
            max number of armies to be randomly generated with GUS
        seed: int = None
            random seed for constant randomization 
        beam_size: int = 1
            size of beam in beam search
        gus_percentage: float = 0.7
            percentage army to be gus
        top_armies_num: int = 25
            number of top armies after GUS
        greedy_levels: List = [1,2,3]
            list of greedy levels
        """
        super().__init__(
            net,
            device,
        ...
        # with 70% of the budget, get the GUS army
        self._gus = GeneralUniformArmySelector(
            self.available_units, 
            y_area=y_area,
            minerals=np.round(minerals * gus_percentage),
            gas=np.round(gas * gus_percentage),
            supply=np.round(supply * gus_percentage),
            units=units,
            seed=seed,
        )
        
        self.max_evals = max_evals
        self.top_armies_num = top_armies_num
        
        # Get the remainder of the army-building resource
        self.remain_minerals = minerals - np.round(minerals*gus_percentage)
        self.remain_gas = gas - np.round(gas*gus_percentage)
        self.remain_supply = supply - np.round(supply*gus_percentage)

        # Create constraint in building army with remain resources
        constraints = build_constraints(
            available_units=available_units, 
            minerals=self.remain_minerals,
            gas=self.remain_gas, 
            supply=self.remain_supply, 
            units=self.units)

        self._army = np.array([x for x, y in constraints], dtype=np.float32)
        self._budget = np.array([y for x, y in constraints], dtype=np.float32) 
        self.greedy_levels= greedy_levels

    def _is_army_within_budget(self, army):
        """Check if the army to be builts is within the remaining resources

        Parameters
        ----------
        army:  List[Union[unit_data.units.Terran, unit_data.units.Protoss, unit_data.units.Zerg]]
            the army to be built

        Return
        ----------
        list of armies marked either within the budget or not
        """
        if army is None:
            return True
        ...
        satisfied = self._army.dot(vec) <= self._budget
        
        return np.all(satisfied)

    def _fringe(self, army):
        """Add the army to the armies if it is within the budget
        ...
        Return
        ----------
        list of within-the-budget armies ready to be added
        """
        if not self._is_army_within_budget(army):
            return
    ...
    def _army_to_tuple(self, army):
        """Convert an army into tuple of units
    ...
    def _tuple_to_army(self, army):
        """Convert a tuple or units into an army
        ...
        Return
        ----------
        an army created by a tuple of units
        """
        ret = []
        for ix, unit in enumerate(self.available_units):
            if army[ix] > 0:
    ...
    def optimize(self, raw_non_self_units, safe_spawn, upgrades):
        """Run the gus algorithms and the greedy search algorithms. 
        This is an abstract method called from ArmySynthesis

        Parameters
        ----------
        raw_non_self_units:
            raw input data of the army
        safe_spawn:
            location on the map to send army
        upgrades:
            information about the upgrade of the army
            
        Return
        ----------
        the best army 
        """ 
        # Create an empty list of armies' win prop
        top_army_evaluation_pair = [] # can used this to sort

        # Generate 100 armies using 70% of the budget with GUS
        samples = [self._gus.select() for _ in range(self.max_evals)]

        # Evaluate the army - we have the win-prop embedded to this
        evals = self.evaluate_armies(samples, raw_non_self_units, safe_spawn, upgrades) # note safe_spawn is put in _initialize_float
        
        # Sort the samples by performance, returned (army, evaluation) pair
        gus_army_evaluation_pair = sorted([(sample, ev) for sample, ev in zip(samples, evals)], key=lambda x: -x[1])

        # Select the top 25 army - 70% budget
        if(len(gus_army_evaluation_pair) >= self.top_armies_num):
            top_army_evaluation_pair.append(gus_army_evaluation_pair[_] for _ in range(self.top_armies_num))
        else:
            top_army_evaluation_pair.append(gus_army_evaluation_pair[_] for _ in range(len(gus_army_evaluation_pair)))

        # Get the top army only instead of tuple (army, evaluation) - how to unzip
        unzipped_pair = zip(*top_army_evaluation_pair)
        unzipped_list = list(unzipped_pair)
        top_incomplete_armies = unzipped_list[0] # this should be a list of top armies

        complete_armies = []

        # For each 25 incomplete army:
        for incomplete_army in top_incomplete_armies: 
            # For each levels of greedy search:
            for levels in self.greedy_levels:
                # Convert incomplete army into units
                fringe = self._fringe(self._army_to_tuple(incomplete_army))

                # while able to add unit into an army - fringe > 0
                while len(fringe) > 0: 

                    # Create an army with number of levels, add 1 or 2 or 3 unit at the same time
                    army = [self._tuple_to_army(fringe[levels-1]) for _ in len(fringe)]

                    # Evaluate that army
                    eval = self.evaluate_armies(armies=army, raw_non_self_units=raw_empty_units, safe_spawn=safe_spawn, upgrades=upgrades)

                    # Tuple the armies with their evaluation and add the army to a list
                    complete_armies.append((self._tuple_to_army(army), eval))

                fringe = [] # reset fringe

        # return sorted of the best army with its network evalution
        return sorted(complete_armies, key=lambda x: -x[1]) 

        # In case want to return just armies
        # a = sorted(complete_armies, key=lambda x: -x[1]) 
        # unzip = zip(*a)
